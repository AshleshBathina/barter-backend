...
export const createPost = async (req, res) => {
  try {
    const { title, type, description, inExchangeFor, longitude, latitude } =
      req.body;
    const { _id } = req.user;
...
export const updatePost = async (req, res) => {
  try {
    ...
    const imagesToDelete = post.images.filter(
      (img) => !keptImageIds.includes(img._id.toString()),
    );
    await deleteImages(imagesToDelete);

    const existingImages = post.images.filter((img) =>
      keptImageIds.includes(img._id.toString()),
    );
...
export const getPosts = async (req, res) => {
  try {
    const { search = "", type, sort, order = "desc" } = req.query;
    ...
    const allowedTypes = ["online", "offline"];
